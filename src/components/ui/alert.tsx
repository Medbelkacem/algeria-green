...
const alertVariants = cva(
  "relative grid w-full grid-cols-[auto_1fr] items-start gap-x-3 gap-y-1 rounded-lg border px-4 py-3 text-sm [&>svg]:size-5 [&>svg]:translate-y-0.5",
  {
    variants: {
      variant: {
        ...
        destructive: "border-destructive/25 bg-destructive/8 text-foreground [&>svg]:text-destructive",
      },
    },
...
function AlertTitle({ className, ...props }: React.ComponentProps<"h5">) {
  return <h5 className={cn("col-start-2 font-medium leading-tight", className)} {...props} />;
}
